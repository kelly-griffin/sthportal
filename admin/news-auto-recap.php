...
function parse_pbp(string $pbp, array $ALIASES): array
{
  ...
  if (preg_match('/\bOT\b|\bovertime\b/i', $plain))
    $out['ot'] = true;
  if (preg_match('/\bSO\b|\bshootout\b/i', $plain))
    $out['so'] = true;
  ...
  if (!$out['star1'] && preg_match('/Three\s+Stars.*?(?:\r?\n|\r)(.*?)(?:\r?\n|\r)(.*?)(?:\r?\n|\r)(.*?)(?:\r?\n|\r)/is', $pbp, $ts)) {
    $strip = function ($s) {
      return trim(preg_replace('/^\s*\d+\s*[-.:]\s*/', '', $s)); };
    $out['star1'] = $strip($ts[1]);
    $out['star2'] = $strip($ts[2]);
    $out['star3'] = $strip($ts[3]);
  }
  if ($out['homeName'] && $out['awayName'] && is_numeric($out['homeScore']) && is_numeric($out['awayScore'])) {
    if ($out['homeScore'] > $out['awayScore']) {
      $out['winner'] = $out['homeName'];
      $out['loser'] = $out['awayName'];
    } elseif ($out['awayScore'] > $out['homeScore']) {
      $out['winner'] = $out['awayName'];
      $out['loser'] = $out['homeName'];
    }
  }
...
{
  if (!$p['winner'] || $p['homeScore'] === null || $p['awayScore'] === null)
    return 'Game Recap';
  $a = (int) $p['homeScore'];
  $b = (int) $p['awayScore'];
  $w = $p['winner'];
  ...
  $dash = "–"; // U+2013 en dash
  if ($p['so'])
    return $w . ' beat ' . $l . ' ' . $hi . $dash . $lo . ' in a shootout';
  if ($p['ot'])
    return $w . ' edge ' . $l . ' ' . $hi . $dash . $lo . ' in OT';
...
<body>
  <div class="site">
    ...
    <div class="canvas">
      <div class="arecap-container">
        <div class="arecap-card">
          <div class="arecap-lead"><span>Auto Game Recap (Paste STHS PBP)</span></div>

          <form class="arecap-form" method="post">
            <label class="arecap-instr"><b>Paste STHS Play-by-Play or Game Summary *</b>
              <textarea name="pbp"
                placeholder="Paste the PBP or game summary here..."><?= htmlspecialchars($pbp) ?></textarea>
              <div class="arecap-help">We’ll parse the winner, score, OT/SO, and Three Stars if present. You can still edit the
                text below.</div>
            </label>
            <div>
            ...
            <?php if ($mode === 'preview'): ?>
              <div class="arecap-title"><span>Preview</span></div>
              <div class="arecap-preview">
                <div class="arecap-row">
                  <label class="arecap-title"><b>Title *</b>
                    <input type="text" name="title" value="<?= htmlspecialchars($title) ?>">
                  </label>
                  <label class="arecap-team"><b>Team (for placeholder image)</b>
                ...
                <label class="arecap-summary"><b>Summary *</b>
                  <input type="text" name="summary" value="<?= htmlspecialchars($summary) ?>">
                </label>
                <label class="arecap-body"><b>Body</b>
