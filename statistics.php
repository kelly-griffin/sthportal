...
if (is_file($draftPicksPath)) {
    ...
    if (is_array($tmp)) $draftPicksPerRoundMap = $tmp;
}
...
function lookupDerivedOtg(string $name, string $teamAbbr, ?array $mapExact = null, ?array $mapNameOnly = null): ?int {
    ...
    if ($mapExact === null)    { $mapExact = isset($derivedOtgMap) ? (array)$derivedOtgMap : []; }
    if ($mapNameOnly === null) { $mapNameOnly = isset($derivedOtgByName) ? (array)$derivedOtgByName : []; }
    $nk = mb_strtolower(trim((string)$name));
    $tk = strtoupper(trim((string)$teamAbbr));
    if ($nk === '') return null;
    if ($tk !== '') {
        $k = $nk . '|' . $tk;
        if (array_key_exists($k, $mapExact)) return (int)$mapExact[$k];
    }
    ...
    if (is_readable($teamsJsonPath)) {
      ...
      if (isset($json['teams']) && is_array($json['teams'])) {
        foreach ($json['teams'] as $t) {
          ...
          if ($full && $abbr) $out[$full] = strtoupper($abbr);
        }
      }
    }
    return $out;
...
<div class="table-scroll">
            <table class="data skaters-table" aria-label="Skaters Summary">
              ...
              <tbody>
                ...
                $rank = 1;
                foreach ($rows as $row): ?>
                  <tr data-name="<?= htmlspecialchars($row['name']) ?>" data-team="<?= htmlspecialchars($row['team']) ?>"
                    data-pos="<?= htmlspecialchars($row['pos']) ?>" data-gp="<?= $row['gp'] ?>" data-g="<?= $row['g'] ?>"
                    data-a="<?= $row['a'] ?>" data-p="<?= $row['p'] ?>" data-plusminus="<?= $row['plusminus'] ?>"
                    data-pim="<?= $row['pim'] ?>" data-p_per_gp="<?= number_format($row['p_per_gp'], 3, '.', '') ?>"
                    data-evg="<?= $row['evg'] ?>" data-ppg="<?= $row['ppg'] ?>" data-ppp="<?= $row['ppp'] ?>"
                    data-shg="<?= $row['shg'] ?>" data-shp="<?= $row['shp'] ?>" data-otg="<?= $row['otg'] ?>"
                    data-gwg="<?= $row['gwg'] ?>" data-s="<?= $row['s'] ?>"
                    data-s_pct="<?= number_format($row['s_pct'], 3, '.', '') ?>" data-toi_gp="<?= $row['toi_gp'] ?>"
...
foreach ($players as $row):
    ...
    if (function_exists('pos_code_from_flags')) {
        ...
        $C=$truthy($row['PosC']??''); $L=$truthy($row['PosLW']??''); $R=$truthy($row['PosRW']??''); $D=$truthy($row['PosD']??'');
...
$draftRound = ($overallInt > 0 && $perRound > 0)
  ...
  : '0';
    ...
    $dobSort = '';
    if ($dobRaw !== '') { $t = strtotime($dobRaw); $dobSort = $t ? date('Y-m-d', $t) : $dobRaw; }
...
?>
  <tr
    ...
    data-dob="<?= htmlspecialchars($dobSort) ?>"
    data-city="<?= htmlspecialchars($city) ?>"
    data-region="<?= htmlspecialchars($region) ?>"
    data-country="<?= htmlspecialchars(strtoupper($country)) ?>"
    data-height="<?= htmlspecialchars($height) ?>"
    data-weight="<?= htmlspecialchars($weight) ?>"
    data-draft_year="<?= htmlspecialchars($draftYear) ?>"
    data-draft_round="<?= (int)$draftRound ?>"
    data-draft_overall="<?= htmlspecialchars($draftOverall) ?>"
    data-gp="<?= (int)$gp ?>"
    data-g="<?= (int)$g ?>"
    data-a="<?= (int)$a ?>"
    data-p="<?= (int)$p ?>"
...
if (!function_exists('load_team_full_to_code')) {
    function load_team_full_to_code(string $teamsJsonPath): array {
      ...
      if (is_readable($teamsJsonPath)) {
        ...
        if (isset($json['teams']) && is_array($json['teams'])) {
          foreach ($json['teams'] as $t) {
            $full = $t['fullName'] ?? ($t['name'] ?? ''); $abbr = $t['abbr'] ?? ($t['triCode'] ?? '');
            if ($full && $abbr) $out[$full] = strtoupper($abbr);
          }
        }
      }
    ...
    function find_pbp_files(array $roots): array {
      ...
      foreach ($roots as $root) {
        ...
        foreach ($it as $f) {
          ...
          if ($ext === 'txt' || $ext === 'html') $out[] = $f->getPathname();
        }
      }
      return $out;
    }
...
<div class="skaters-subpanel" data-subtab="faceoffs" hidden>
  <div class="table-scroll">
    <table class="bio-table" aria-label="Skaters Face-off Percentages">
      ...
      $rank = 1;
      foreach ($players as $r):
        $name = getName($r, $aliases); if ($name==='') continue;
        ...
        $pbp = isset($FO_PBP[$k]) ? $FO_PBP[$k] : [
          'total_won'=>0,'total_taken'=>0,
          'ev_won'=>0,'ev_taken'=>0,'pp_won'=>0,'pp_taken'=>0,'sh_won'=>0,'sh_taken'=>0,
          'oz_won'=>0,'oz_taken'=>0,'nz_won'=>0,'nz_taken'=>0,'dz_won'=>0,'dz_taken'=>0,
        ];
        if ($gp === 0) {
          $pbp = [
            'total_won'=>0,'total_taken'=>0,
...
$ppTaken = (int)$pbp['pp_taken']; $ppWon = (int)$pbp['pp_won'];
$shTaken = (int)$pbp['sh_taken']; $shWon = (int)$pbp['sh_won'];
...
$evWon   = max(0, $totalWon   - $ppWon   - $shWon);
        $ozTaken = (int)$pbp['oz_taken']; $ozWon = (int)$pbp['oz_won'];
        $nzTaken = (int)$pbp['nz_taken']; $nzWon = (int)$pbp['nz_won'];
        $dzTaken = (int)$pbp['dz_taken']; $dzWon = (int)$pbp['dz_won'];
      ...
      ?>
        <tr
          ...
          data-pos="<?= htmlspecialchars($pos) ?>"
          data-gp="<?= (int)$gp ?>"
          data-toi_gp="<?= htmlspecialchars($toi_gp_str) ?>"
          data-fo="<?= (int)$totalTaken ?>"
          data-ev_fo="<?= (int)$evTaken ?>"
          data-pp_fo="<?= (int)$ppTaken ?>"
          data-sh_fo="<?= (int)$shTaken ?>"
          data-oz_fo="<?= (int)$ozTaken ?>"
          data-nz_fo="<?= (int)$nzTaken ?>"
          data-dz_fo="<?= (int)$dzTaken ?>"
          data-fow_pct="<?= $fowPct ?>"
          data-ev_fow_pct="<?= $evPct ?>"
          data-pp_fow_pct="<?= $ppPct ?>"
          data-sh_fow_pct="<?= $shPct ?>"
          data-oz_fow_pct="<?= $ozPct ?>"
          data-nz_fow_pct="<?= $nzPct ?>"
          data-dz_fow_pct="<?= $dzPct ?>"
        >
          <td class="col-rank"><?= $rank++ ?></td>
          <td class="col-name"><?= htmlspecialchars($name) ?></td>
      ...
      <?php if (empty($players)): ?>
        <tr><td colspan="19" style="text-align:center; opacity:.7; padding:12px 6px;">No skater rows available</td></tr>
      <?php endif; ?>
      </tbody>
    </table>
  </div>
...
foreach ($players as $row):
    ...
    $ozTaken = (int)$pbp['oz_taken']; $ozWon = (int)$pbp['oz_won']; $ozLost = max(0, $ozTaken - $ozWon);
    $nzTaken = (int)$pbp['nz_taken']; $nzWon = (int)$pbp['nz_won']; $nzLost = max(0, $nzTaken - $nzWon);
    $dzTaken = (int)$pbp['dz_taken']; $dzWon = (int)$pbp['dz_won']; $dzLost = max(0, $dzTaken - $dzWon);
?>
        <tr
          data-name="<?= htmlspecialchars($name) ?>"
          data-team="<?= htmlspecialchars($team) ?>"
          ...
          data-gp="<?= (int)$gp ?>"
          data-fo="<?= (int)$totalTaken ?>"
          data-fow="<?= (int)$totalWon ?>"
          data-fol="<?= (int)$totalLost ?>"
          data-fow_pct="<?= $fowPct ?>"
          data-ev_fo="<?= (int)$evTaken ?>"
          data-ev_fow="<?= (int)$evWon ?>"
          data-ev_fol="<?= (int)$evLost ?>"
          data-pp_fo="<?= (int)$ppTaken ?>"
          data-pp_fow="<?= (int)$ppWon ?>"
          data-pp_fol="<?= (int)$ppLost ?>"
          data-sh_fo="<?= (int)$shTaken ?>"
          data-sh_fow="<?= (int)$shWon ?>"
          data-sh_fol="<?= (int)$shLost ?>"
          data-oz_fo="<?= (int)$ozTaken ?>"
          data-oz_fow="<?= (int)$ozWon ?>"
          data-oz_fol="<?= (int)$ozLost ?>"
          data-nz_fo="<?= (int)$nzTaken ?>"
          data-nz_fow="<?= (int)$nzWon ?>"
          data-nz_fol="<?= (int)$nzLost ?>"
          data-dz_fo="<?= (int)$dzTaken ?>"
          data-dz_fow="<?= (int)$dzWon ?>"
...
<?php if (empty($players)): ?>
        <tr
          ...
          data-dz_fow="<?= (int)$dzWon ?>"
          data-dz_fol="<?= (int)$dzLost ?>"
        ><td colspan="27" style="text-align:center; opacity:.7; padding:12px 6px;">No skater rows available</td></tr>
<?php endif; ?>
      </tbody>
    </table>
  </div>
      ...
      <section id="tab-goalies" class="tab-panel" aria-labelledby="goalies">
<!-- Goalies – Summary -->
...
<?php
  ...
  if (!function_exists('pct1')) {
    function pct1($num,$den){ return $den>0 ? number_format($num*100/$den, 1) : '0.0'; }
  }
  if (!function_exists('fmt_hms')) {
    function fmt_hms($s){ $h=intval($s/3600); $m=intval(($s%3600)/60); $ss=intval($s%60); return sprintf('%d:%02d:%02d',$h,$m,$ss); }
  }
  ...
  foreach (($goalies ?? []) as $row):
    ...
    // GP==0 guard
    if ($gp === 0) { $gs=$w=$l=$ot=$sa=$ga=$sec=$so=$a=$pim=0; }
...
?>
  ...
  data-name="<?= htmlspecialchars($name) ?>"
  data-team="<?= htmlspecialchars($team) ?>"
  data-gp="<?= $gp ?>" data-gs="<?= $gs ?>"
  data-w="<?= $w ?>"  data-l="<?= $l ?>" data-ot="<?= $ot ?>"
  data-sa="<?= $sa ?>" data-svs="<?= $svs ?>" data-ga="<?= $ga ?>"
...
<?php if (empty($goalies ?? [])): ?>
        <tr><td colspan="17" style="text-align:center; opacity:.7; padding:12px 6px;">No goalie rows available</td></tr>
<?php endif; ?>
      </tbody>
    </table>
  </div>
...
<!-- TEAMS TAB -->
      ...
      <section id="tab-teams" class="tab-panel" aria-labelledby="teams">
...
if (!function_exists('safe_div')) { function safe_div($n,$d){ return $d>0 ? ($n/$d) : 0.0; } }
if (!function_exists('fmt1')) { function fmt1($v){ return number_format((float)$v, 1); } }
if (!function_exists('fmt2')) { function fmt2($v){ return number_format((float)$v, 2); } }
if (!function_exists('fmt3')) { function fmt3($v){ return number_format((float)$v, 3); } }
  ...
  function value_of($row, $key, $aliases, $default='') {
    ...
    if (isset($aliases[$key])) {
      foreach ($aliases[$key] as $alt) {
        if (isset($row[$alt]) && $row[$alt] !== '') return $row[$alt];
      }
    }
    return $default;
  }
