<?php
/**
 * tools/fetch_headshot.php — v0.2
 * One-off fetch to your chosen destination.
 * Examples:
 *   /tools/fetch_headshot.php?id=8471214&prefer=mugs&dest=flat&flat_dir=assets/img/mugs
 *   /tools/fetch_headshot.php?id=8471214&team=WSH&teamnum=2&dest=abbr
 */
...
$prefer = isset($_GET['prefer']) ? strtolower($_GET['prefer']) : 'mugs'; // default mugs for one-offs
$dest   = isset($_GET['dest']) ? strtolower($_GET['dest']) : 'flat'; // default flat
...
if ($dest!=='flat' && !$team && !$teamnum){ echo "ERR: team or teamnum required unless dest=flat\n"; exit; }

$destRel = ($dest==='flat')
  ? "{$flatDir}/{$id}.png"
  : ("data/uploads/headshots/".(($dest==='num' && $teamnum!=='')?$teamnum:$team)."/{$id}.png");
...
if (save_file($destAbs,$bytes)){
  echo "OK: saved from " . strtoupper($src) . " to /{$destRel}\n";
} else {
  echo "ERR: could not write to /{$destRel}\n";
}
