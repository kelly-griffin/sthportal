...
function tx_move_side_label(array $m, array $code2abbr): array {
  $name = ''; $kind = ''; // kind: 'to' or 'from'
  if (!empty($m['to']))        { $name = $m['to'];   $kind = 'to'; }
  elseif (!empty($m['from']))  { $name = $m['from']; $kind = 'from'; }

  if ($name === '') { // fallbacks for non-team “destinations”
    $action = strtolower($m['action'] ?? '');
    if (str_contains($action, 'waiver'))          { $name = 'Waivers';        $kind = 'to'; }
    elseif (str_contains($action, 'bought out'))  { $name = 'Free Agents';    $kind = 'to'; }
    elseif (str_contains($action, 'placed on ltir')) { $name = 'Long-Term IR'; $kind = 'to'; }
    elseif (str_contains($action, 'placed on ir'))   { $name = 'Injured Reserve'; $kind = 'to'; }
    elseif (str_contains($action, 'activated'))      { $name = 'Active Roster';   $kind = 'to'; }
  }
...
// Draft pick mapping: Y1=2025, Y2=2026, Y3=2027, ...
        ...
        <div class="transactions-card">
          ...
          <section id="tab-trades" class="tab-panel active">
            <div class="tx-toolbar">
              ...
              <label>Team
                <select id="trades-team">
                  ...
                  <?php
                    $opts = [];
                    foreach ($trades as $t) { $opts[$t['from']] = true; $opts[$t['to']] = true; }
                    foreach (array_keys($opts) as $c) echo '<option>'.htmlspecialchars($c).'</option>';
                  ?>
                </select>
              </label>
            ...
            <ul class="tx-list" id="trades-list">
              <?php if (empty($trades)): ?>
                <li class="tx-card"><div class="tx-mid">No trades yet.</div></li>
              <?php else: foreach ($trades as $t): ?>
<li class="tx-card trade-card"
    data-date="<?= htmlspecialchars($t['date']) ?>"
    data-from="<?= htmlspecialchars($t['from']) ?>"
    data-to="<?= htmlspecialchars($t['to']) ?>"
...
<div class="assets assets-left">
  ...
  <div class="tx-toolbar">
    ...
    <label>Team
      <select id="signings-team">
        ...
        <?php
          $opts = [];
          foreach ($signings as $s) { $opts[$s['team']] = true; }
          foreach (array_keys($opts) as $c) echo '<option>'.htmlspecialchars($c).'</option>';
        ?>
      </select>
    </label>
  ...
  <ul class="tx-list" id="signings-list">
    <?php if (empty($signings)): ?>
      <li class="tx-card"><div class="tx-mid">No signings yet.</div></li>
    <?php else: foreach ($signings as $s): ?>
      <li class="tx-card signing-card"
          data-team="<?= htmlspecialchars($s['team']) ?>"
        ...
        <div class="tx-note-row">
          <span class="tx-note"><?= htmlspecialchars($m['notes']) ?></span>
        </div>
        <?php endif; ?>
        <?php endif; ?>
          ...
          <section id="tab-moves" class="tab-panel">
            <div class="tx-toolbar">
              ...
              <label>Team
                ...
                <?php
          $opts = [];
          foreach ($moves as $m) { if (!empty($m['team'])) $opts[$m['team']] = true; }
          foreach (array_keys($opts) as $c) echo '<option>'.htmlspecialchars($c).'</option>';
        ?>
      </select>
    </label>
  ...
  <ul class="tx-list" id="moves-list">
    <?php if (empty($moves)): ?>
      <li class="tx-card"><div class="tx-mid">No roster moves yet.</div></li>
    <?php else: foreach ($moves as $m): ?>
      <li class="tx-card tx-move move-card"
          data-team="<?= htmlspecialchars($m['team']) ?>"
        ...
        <div class="tx-card-main">
          ...
          </div>          <!-- Date pill on left (like trades) -->
...
<?php if (!empty($m['tags']) || !empty($m['notes'])): ?>
<?php if (!empty($m['notes'])): ?>
        <div class="tx-note-row">
          <span class="tx-note"><?= htmlspecialchars($m['notes']) ?></span>
        </div>
